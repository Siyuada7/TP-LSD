...
# path for wireframe image
image_p = "data/Wireframe/coco/images/val2017/"
suffix = '.png'
GT = "data/Wireframe/linemat/"
...
def afm_mat(path, threshold=10, reverse=True):
    preds = sorted(glob.glob(path))
    gts = sorted(glob.glob(GT))
    sumtp, sumfp = np.array([0]), np.array([0])
    n_gt = 0
...
def wireframe_score(WF, T=10):
    dirs = [2, 6, 10, 20, 30, 50, 80, 100, 150, 200, 250, 255]

    precision, recall = [], []
    for threshold in dirs:
        print("Processing", threshold)
        mat_files = os.listdir(osp.join(WF, str(threshold)))
        mat_files.sort()
        tp, fp, total_gt = 0, 0, 0
        for pred_name in mat_files:
            if '.jpg' in pred_name:
                continue
            name = pred_name.split('/')[-1].replace(".mat", suffix)
            img = cv2.imread(image_p + name) # wire jpg
            height, width = img.shape[:2]
            pred_path = osp.join(WF, str(threshold), pred_name)
            line_pred = scipy.io.loadmat(pred_path)["lines"].reshape(-1, 2, 2)
            ...
            gt_line[:, 1, 0] = tmp[:, 1, 1] / height * 128
            gt_line[:, 1, 1] = tmp[:, 1, 0] / width * 128
            total_gt += len(gt_line)


            tp_, fp_ = msTPFP(line_pred, gt_line, T)
            tp += tp_.sum()
            fp += fp_.sum()
            total_gt += len(gt_line)
    ...
    # np.savez(
    #     "/data/lcnn/result/sAP/FScore.npz",
...
def lcnn_score(path, lcnn=True, threshold=10):
    ...
    for gt_name in gts:
        if len(gt_name) < 3:
            continue
        #print(gt_name)
        img = cv2.imread(image_p + gt_name.split('/')[-1].replace("_line.mat", suffix))
        height, width = img.shape[:2]
        pred_name = path + '/' + str(num_pic).rjust(6, '0') + '.npz'
        num_pic += 1
        ...
        #gt_name = GT + mat_name.split('/')[-1].replace(".npz", "_line.mat")
        mat = sio.loadmat(gt_name)
        gt_line = mat['lines'].reshape(-1, 2, 2)
...
if __name__ == "__main__":
    scale = 128
    # afm = '/AFM/wire/atrous/scores/*.mat'
    # lsd = '/LSD/wire/lsd_scores/*.mat'
    # wire = '/DWP/wire/'
    # lcnn = '/LCNN/wire/logs/pretrained-model/npz/000312000/'
    # lcnn_p = '/LCNN/wire/post/pretrained-model-000312000/0_010/'

    test = 'log/test/wire/mat/lmbd0.5/0.01/*.mat'
